...
function App() {
  ...
  const selectHistory = (item) => {
    ...
    setResults({
      ...
      optimalAnswer: item.summary, // In this version summary and optimalAnswer are similar
      validationReport: "히스토리에서 불러온 데이터입니다. 상세 검증 리포트는 실시간 분석시에만 생성됩니다.",
      heroImage: null
    });
    ...
    <div className={`min-h-screen transition-all duration-700 ease-in-out ${theme === 'dark' ? 'bg-[#050507] text-slate-200' : 'bg-[#fcfdfe] text-slate-900'} font-sans selection:bg-indigo-500/30 overflow-x-hidden`}>
      <div className="max-w-7xl mx-auto p-4 md:p-10 space-y-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 pt-4">
          ...
          {/* Sidebar Area: Live Activity & History Tabbed */}
            ...
            <div className={`bg-white dark:bg-[#0d0d10] border-2 ${theme === 'dark' ? 'border-slate-800/50' : 'border-slate-100'} rounded-[3rem] p-8 h-[800px] flex flex-col shadow-2xl relative overflow-hidden transition-all duration-700 shadow-black/5`}>
              ...
              <div className="flex bg-slate-50 dark:bg-black/20 p-1.5 rounded-[1.5rem] mb-8">
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-[1rem] text-[10px] font-black uppercase tracking-widest transition-all ${activeSidebarTab === 'live' ? 'bg-white dark:bg-[#1a1a1f] text-indigo-500 shadow-lg' : 'text-slate-400'}`}
                >
                ...
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-[1rem] text-[10px] font-black uppercase tracking-widest transition-all ${activeSidebarTab === 'history' ? 'bg-white dark:bg-[#1a1a1f] text-indigo-500 shadow-lg' : 'text-slate-400'}`}
                >
                    ...
                    >
                      {timeline.length === 0 && !isAnalyzing && (
                        <div className="text-center py-40 text-slate-200 dark:text-slate-800 space-y-6">
                          ...
                          <p className="text-[10px] font-black tracking-[0.4em] opacity-40 uppercase">Ready to Orchestrate</p>
                        </div>
                      )}
                    ...
                    >
                      {history.length === 0 ? (
                        <div className="text-center py-40 text-slate-200 dark:text-slate-800 space-y-6">
                          ...
                          <p className="text-[10px] font-black tracking-[0.4em] opacity-40 uppercase">No History Found</p>
                        </div>
                      ) : (
                        history.map((item) => (
                          <div
                            ...
                            className="bg-slate-50/50 dark:bg-white/5 border border-slate-100 dark:border-white/5 rounded-[1.5rem] p-5 cursor-pointer hover:border-indigo-500/30 transition-all hover:bg-indigo-50/10 group relative"
                          >
                            <p className="text-[12px] font-black text-slate-700 dark:text-slate-200 mb-2 line-clamp-2 pr-6">
                              {item.prompt}
                            </p>
              ...
              <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                <div className="flex flex-wrap gap-4 p-2 bg-white dark:bg-[#0d0d10] border-2 border-slate-100 dark:border-slate-800/50 rounded-[2.2rem] shadow-xl">
                  {[
                    ...
                    <button
                      ...
                      className={`relative flex items-center gap-4 px-8 py-4 rounded-[1.6rem] text-[13px] font-black transition-all z-10 ${activeTab === tab.id ? 'text-white' : 'text-slate-400'}`}
                    >
                      {activeTab === tab.id && (
                        <motion.div
                          layoutId="premium-active-tab"
                          className="absolute inset-0 bg-gradient-to-br from-indigo-600 via-violet-600 to-purple-600 shadow-lg rounded-[1.6rem] -z-10"
                        />
                      )}
                      <tab.icon className={`w-4.5 h-4.5 ${activeTab === tab.id ? 'scale-110' : ''}`} />
                ...
                <div className="flex items-center gap-4">
                  <button
                    ...
                    className="flex items-center gap-4 px-8 py-5 bg-[#000000] dark:bg-white text-white dark:text-black rounded-[1.8rem] font-black text-xs uppercase tracking-widest hover:scale-105 active:scale-95 transition-all disabled:opacity-50"
                  >
                    {isNotionSaving ? (
              ...
              <AnimatePresence mode="wait">
                {isAnalyzing && (
                  <motion.div
                    ...
                    className={`flex flex-col items-center justify-center flex-1 space-y-20 py-40 bg-white dark:bg-[#0d0d10] rounded-[4rem] border-2 ${theme === 'dark' ? 'border-slate-800/50' : 'border-slate-100'} shadow-2xl relative overflow-hidden`}
                  >
                    ...
                    <div className="text-center space-y-8 max-w-2xl px-12 relative z-10">
                      <h3 className="text-5xl font-black text-slate-800 dark:text-white tracking-tighter uppercase leading-tight">Orchestrating <span className="text-indigo-600 underline decoration-indigo-200 underline-offset-8">Intelligence</span></h3>
                      <p className="text-slate-400 dark:text-slate-500 text-lg font-bold">GPT-4, Claude 3.5, Gemini Pro, Perplexity 에이전트가 데이터 수집 및 상호 검증 작업을 수행하고 있습니다.</p>
                    </div>
                  </motion.div>
                )}
                  ...
                  <motion.div key={activeTab} initial={{ opacity: 0, scale: 0.99, y: 30 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: 0.8 }} className="w-full flex-1">
                    {activeTab === 'optimal' && (
                      <div className="space-y-12">
                        {results.heroImage && (
                          <div className="relative h-[400px] w-full overflow-hidden rounded-[4rem] shadow-2xl group border-8 border-white dark:border-[#0d0d10]">
                            <img src={results.heroImage} className="w-full h-full object-cover transition-transform duration-[3000ms] group-hover:scale-110" />
                            <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent to-transparent opacity-90"></div>
                            <div className="absolute bottom-12 left-12">
                              <h2 className="text-5xl font-black text-white tracking-tighter">종합 인텔리전스 분석</h2>
                            </div>
                          </div>
                        )}
                        <div className="bg-white dark:bg-[#0d0d10] border-2 border-slate-100 dark:border-slate-800/50 rounded-[4rem] p-16 shadow-2xl relative overflow-hidden transition-all duration-700">
                    ...
                    {activeTab === 'report' && (
                      <div className="bg-white dark:bg-[#0d0d10] border-2 border-slate-100 dark:border-slate-800/50 rounded-[4rem] p-20 space-y-16 shadow-2xl">
                        <div className="flex items-center justify-between border-b-4 border-slate-50 dark:border-slate-900 pb-12">
                          <div className="flex items-center gap-8">
                            <div className="p-6 bg-gradient-to-br from-indigo-600 to-violet-600 rounded-[2.5rem] shadow-xl shadow-indigo-600/30">
                              <Brain className="w-12 h-12 text-white" />
                            </div>
                            <h2 className="text-5xl font-black text-slate-800 dark:text-white tracking-tighter uppercase italic">분석 리포트</h2>
                          </div>
                        </div>
                        <div className="bg-slate-50/50 dark:bg-[#0a0a0c] p-16 rounded-[3.5rem] border-2 border-slate-100 dark:border-slate-800">
                ...
                {!isAnalyzing && !results && (
                  <div className="flex flex-col items-center justify-center flex-1 py-32 border-4 border-dashed border-slate-100 dark:border-slate-900 rounded-[5rem] bg-white dark:bg-black/5 shadow-inner group relative overflow-hidden">
                    ...
                    <motion.div animate={{ y: [0, -20, 0] }} transition={{ duration: 6, repeat: Infinity }} className="relative z-10">
                      <Bot className="w-32 h-32 opacity-[0.05] dark:opacity-[0.1] mb-8 text-slate-900 dark:text-white" />
                    </motion.div>
                    <p className="text-xs font-black tracking-[0.5em] text-slate-300 dark:text-slate-700 uppercase">Neural Hub Idle</p>
                    <p className="text-base font-bold text-slate-400 dark:text-slate-500 mt-6 italic">Enter a prompt to begin orchestration</p>
                  </div>
                )}
              </AnimatePresence>
            </div>
      ...
      <style dangerouslySetInnerHTML={{
        ...
        .custom-scrollbar::-webkit-scrollbar-thumb { background: ${theme === 'dark' ? '#1e1e24' : '#e2e8f0'}; border-radius: 20px; }
        .prose { font-size: 1.1rem; }
        .prose h2 { font-weight: 900; letter-spacing: -0.06em; margin-top: 3rem; border-left: 6px solid #4f46e5; padding-left: 1.5rem; }
        .prose p { margin-bottom: 2rem; line-height: 1.8; font-weight: 500; }
        .prose strong { color: #6366f1; font-weight: 800; }
      `}} />
    </div>
  );
}
